...
use crate::extras::adviser::builder;
use crate::extras::adviser::{clone_adviser_event_tx, get_session_snapshot, with_config};
use crate::extras::truncate::truncate_cjk;
use crate::permission::ask::AskSender;
use crate::permission::checker::PermCheck;

const ADVISER_TIMEOUT: Duration = Duration::from_secs(300);

const MAX_ADVISER_RESPONSE_BYTES: usize = 16 * 1024;

#[derive(Deserialize)]
pub struct AdviserArgs {
    #[serde(default)]
    pub query: Option<String>,
}

#[derive(Clone)]
pub struct AdviserTool {
...
impl Tool for AdviserTool {
    const NAME: &'static str = "adviser";
    type Error = ToolError;
    type Args = AdviserArgs;
    type Output = String;
...
The adviser sees the full conversation history and returns a plan, correction, \
...
If absent, the adviser infers what you need from context."
                .to_string(),
            parameters: serde_json::json!({
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Optional specific question to focus the adviser on. If absent, the adviser infers what's needed from the conversation context."
                    }
                },
                "required": []
            }),
    ...
    async fn call(&self, args: AdviserArgs) -> Result<String, ToolError> {
        check_perm(
            &self.permission,
            &self.ask_tx,
            Self::NAME,
        ...
        let adviser_event_tx = clone_adviser_event_tx();
        ...
        // overloading the adviser's context window while keeping enough
        ...
        for msg in &messages[start_idx..] {
            match msg.role {
                ...
                crate::session::MessageRole::Assistant => {
                    ...
                    // blowing up the adviser context
        ...
        if let Some(tx) = &adviser_event_tx {
            let _ = tx.send(crate::event::AgentEvent::AdviserConsulting).await;
        }

        let work = async {
            let agent = builder::build_adviser_agent(model, max_turns).await;
            agent
                .run_subagent(&conversation, max_turns, adviser_event_tx.as_ref())
                .await
        };

        let result = match tokio::time::timeout(ADVISER_TIMEOUT, work).await {
            Ok(Ok(response)) => response,
            Ok(Err(e)) => {
                if let Some(tx) = &adviser_event_tx {
                    let _ = tx
                        .send(crate::event::AgentEvent::AdviserError {
                            error: compact_str::CompactString::new(format!("error: {}", e)),
                        })
                        .await;
                }
                return Err(ToolError::Msg(format!("adviser error: {}", e)));
            }
            Err(_elapsed) => {
                if let Some(tx) = &adviser_event_tx {
                    let _ = tx
                        .send(crate::event::AgentEvent::AdviserError {
                            error: compact_str::CompactString::new(
                                "timeout: adviser exceeded 300s",
                            ),
                        })
                        .await;
                }
                return Err(ToolError::Msg(
                    "adviser timeout: adviser exceeded 300s".to_string(),
                ));
            }
        };
        ...
        let truncated = truncate_cjk(
            &result,
            MAX_ADVISER_RESPONSE_BYTES,
            &format!(
                "\n…[adviser response truncated at {}B]",
                MAX_ADVISER_RESPONSE_BYTES
            ),
        );
        ...
        if let Some(tx) = &adviser_event_tx {
            let _ = tx
                .send(crate::event::AgentEvent::AdviserResult {
