...
You are a strategic advisor to a coding agent. The agent is currently working on a \
...
- Do NOT generate code or write files. You are a pure advisor.
- Do NOT address the user directly. Address the agent in second person.";

/// Prompt block appended to the main agent's system prompt when the advisor
/// feature is enabled. Tells the executor when and how to use the advisor tool.
pub(crate) const ADVISOR_TOOLS_PROMPT: &str = "\
\n- **advisor**: Call a stronger model for strategic guidance. Call before \
committing to a complex approach, when stuck on an error, when considering a \
change of approach, or when the task appears complete. The advisor sees the \
