...
impl Config {
    ...
    /// Whether requests for `provider` + `model_id` go through the
    /// Anthropic-native API route. This is the route that enables prompt
    /// caching and reports `input_tokens` *excluding* cached/cache-creation
    /// tokens, so it is the only one whose context accounting must add the
    /// cache fields back in (see [`Session::real_input_tokens`](crate::session::Session::real_input_tokens)).
    ///
    ...
    ///
    /// OpenCode Zen/Go serve Anthropic-native models (Claude, Qwen, MiniMax on
    /// the messages route) next to OpenAI-protocol ones under a single
    /// provider id, so for those the per-model transport decides.
    pub fn is_anthropic_native(&self, provider: &str, model_id: &str) -> bool {
        let kind_name = self
            .custom_providers
            .as_ref()
        ...
        matches!(
            crate::provider::opencode_transport(provider, model_id),
            Some(crate::provider::OpencodeTransport::Messages)
        )
    }
