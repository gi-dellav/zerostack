...
| `mouse_capture`           | boolean | When `true` (default), the TUI captures mouse events so zerostack can handle scrolling, click-to-place-cursor, and drag-to-select/copy. When `false`, mouse selection/copy/paste is handled by your terminal emulator instead, but in-app mouse features are disabled. |
